...
public class AffinityLock implements Closeable {
    ...
    private static long getReservedAffinity0() {
        ...
        if (reservedAffinity == null || reservedAffinity.trim().isEmpty()) {
            ...
            if (reserverable == 0 && PROCESSORS > 1) {
                LoggerFactory.getLogger(AffinityLock.class).info("No isolated CPUs found, so assuming CPUs 0 to {} available.", (PROCESSORS - 1));
    ...
    public static AffinityLock acquireCore(boolean bind, int cpuId){
        return acquireCore(bind, cpuId, AffinityStrategies.SAME_CPU_ID,AffinityStrategies.DIFFERENT_CORE,AffinityStrategies.ANY);
    }
