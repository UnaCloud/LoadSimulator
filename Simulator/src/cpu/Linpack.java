...
public class Linpack extends Thread {

    private final int cpuId;
    private AffinityLock af;
    private int percent;
    private final StopWatch watch;
    private boolean keepRunning;
    private int currentload;

    public Linpack(int percent, int cpuId) {
    ...
    private void currentload() {
        ...
        try {
            cpuload = Runtime.getRuntime().exec("WMIC /Output:STDOUT CPU get LoadPercentage /format:LIST");
            BufferedReader br = new BufferedReader(new InputStreamReader(cpuload.getInputStream()));
            for (int i = 0; i < 4; i++) {
                br.readLine();
            }

            currentload = Integer.parseInt(br.readLine().substring(15));
    ...
    @Override
    public void run() {
        af = AffinityLock.acquireCore(true, cpuId);
        watch.start();
        while (keepRunning) {
            if (watch.getElapsedTime() > percent) {
                try {
